...
- `migrations/0003_cards_readmodel.sql` — denormalizes display fields onto `cards` and adds a unique
  `content_hash` so the worker can upsert cards idempotently and the feed reads from one table.
